...
function search(search) {
  $(".movies").remove();
  $(".tv").remove();
  //http://api.themoviedb.org/3/search/multi?api_key=85ee21f7cdf8d07a6d1bb46578bebd06&query=
  var searchurl =
    "https://api.themoviedb.org/3/search/multi?api_key=85ee21f7cdf8d07a6d1bb46578bebd06&query=";
...
function movieInfo(id) {
  ...
  $.getJSON(movieCast + id + "/casts?" + key, function (json) {
    ...
    $.getJSON(infoURL, function (data) {
      ...
      // var recommendationURL = "/recommend/" + 1 + "/";
      var recommendationURL = "/recommend/movie/" + id + "/";
      $.getJSON(recommendationURL, function (recommendations) {
        ...
        // You can process the recommendations here (e.g., display them on the page)
        
        $(".movie-inner").append(
          " <br> <h1>You may like</h1> <br><div class='movie-row' style='display: flex; justify-content:space-evenly;'></div>  "
          );
          
          // $(".movie-row").prepend("");
        for (var i = 0; i < recommendations.length; i++) {
          // $(".movie-inner").append(
          var id = recommendations[i].id;
          var title = recommendations[i].title;
          var overview = recommendations[i].overview;
          var rating = recommendations[i].vote_average;
          var poster = posterPaths + recommendations[i].poster_path;
          if (
            ...
            poster ===
            "https://image.tmdb.org/t/p/w370_and_h556_bestv2undefined"
          ) {
            //dont show if the overview is null
          } else if (overview == "null") {
            //dont show if the overview is null
          } else {
            // Create a new div inside .movie-inner and set it as flex container
            
            // Append items to the new flex container
            $(".movie-inner .movie-row").append(
              "<a class='item link movies m" +
                i +
                " recommend' id='" +
                id +
                "' onclick='movieInfo(" +
                id +
                ")' href='#' style='display: inline-block; height: 400px; width: 290px; margin-right: 20px;'><img src='" +
                poster +
                "' class='image' style='height: 100%; width: 100%; object-fit: cover;'><div class='item-inner' style='text-align: center;'><h2 class='item-title' style='margin-top: 10px;'>" +
                title +
                "</h2><span class='rating'><i class='fa fa-star' aria-hidden='true'></i> " +
                rating +
                "</span></div></a>"
...
function tvInfo(id) {
  ...
  $.getJSON(infoURL, function (data) {
    ...
    // var recommendationURL = "/recommend/" + 1 + "/";
    var recommendationURL = "/recommend/tv/" + id + "/";
    $.getJSON(recommendationURL, function (recommendations) {
      // Handle recommendations
      console.log(
        "Recommendations:",
